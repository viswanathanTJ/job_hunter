...
import { JOBS_FILTER_KEY } from './Jobs.jsx';

/** Back-link target that restores whatever filters the Jobs list was last using. */
function jobsListHref() {
  try {
    const saved = sessionStorage.getItem(JOBS_FILTER_KEY);
  ...
  return (
    <div className="page">
      <div className="page-head compact">
        <div className="detail-head">
          <div>
            <div className="microlabel" style={{ marginBottom: 6 }}>
              <Link to={jobsListHref()} style={{ textDecoration: 'none', color: 'inherit' }}>
                ← jobs
              </Link>{' '}
              / #{job.id}
            </div>
            ...
            <div className="page-sub" style={{ marginBottom: 10 }}>
              ...
              {job.posted_at && <span title={postedExact(job.posted_at)}> · posted {postedLabel(job.posted_at)}</span>}
              ...
              {job.status !== 'applied' && (
                <>
                  <button
                    ...
                    onClick={call(() => apiPost(`/jobs/${id}/proceed`))}
                    disabled={job.status === 'ready_to_apply'}
                  >
                    Proceed →
                  </button>
        ...
        {error && (
          <div className="error-line" style={{ marginBottom: 14 }}>
            {error}
          </div>
        )}
        {job.application && (
          <div className="panel" style={{ marginBottom: 16, borderColor: 'var(--mark-good)' }}>
            ...
            <span style={{ fontSize: 13.5 }}>
              ...
              {job.application.tracker_num ? ` · tracker report #${job.application.tracker_num}` : ' · not yet in tracker'}
            ...
            <div className="panel">
              ...
              {!job.resume && !generating && (
                <p style={{ color: 'var(--ink-3)', fontSize: 13.5 }}>
                  No resume yet. Generation tailors <span className="mono">Resume/resume.html</span> to this JD — truthfully, one page, saved to{' '}
                  <span className="mono">Resume/&lt;Company&gt;/</span>.
                </p>
              )}
              {job.resume && (
                <>
                  ...
                  {hasPdf && (
                    <iframe key={frameKey} className="resume-frame" title="Resume preview" src={`/api/jobs/${id}/resume/file?type=pdf#toolbar=0`} />
                  )}
                </>
              )}
            </div>
      ...
      {showApplied && (
        <Modal title="Mark as applied" onClose={() => setShowApplied(false)}>
          <p style={{ margin: 0, fontSize: 13.5, color: 'var(--ink-2)' }}>
            This records the application, and syncs it into the career-ops tracker (<span className="mono">data/applications.md</span>) with an
